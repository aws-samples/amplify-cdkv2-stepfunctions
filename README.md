...
The example utilizes the Amplify CDK, Storage and Lambda function functionality to deploy a StepFunction that Sync’s S3 buckets.   The following example outlines a use case to copy all objects from a source bucket into a destination bucket(two way), but leave out objects that are already present. 

The following information outlines the steps needed in creating the application.

1. Create a Amplify project
2. Adding a S3 storage resources via Amplify and AWS console. 
...
Lets create a Amplify project and install the necessary packages by running the following commands.
...
```
 ...
 Next let’s add Cognito as this is required by S3 storage. Run the command:
...
### Let’s add an S3 bucket on the AWS console. 
...
We will be creating 4 functions that will be utilized in the step function.

#### Lambda function to retrieve Amplify S3 bucket object names 

The function will have access to the Amplify generated S3 storage that will read the object file names. 

Run the command and select the following prompts
...
```
Note: Replace the <> with the name of the s3 bucket generated in the environment parameters.


#### Lambda function to retrieve AWS console S3 bucket object names 

Similar to the above function added we will add function without the additional permission to s S3 bucket instead provide the permission to the function via custom-policies.json
...
The add the following code to the `index.js` file.
...
Note: Replace the <> with the name of the s3 bucket created in the AWS console.

Add the fillowing package to `package.json`

```json
"aws-sdk": "^2.814.0"
```

Note: for this example I will be setting the MaxKeys to 100 for efficiency, the attribute can be set to a maximum of 1,000 key names. A *continuation-token* (https://docs.aws.amazon.com/AmazonS3/latest/API/API_ListObjectsV2.html#API_ListObjectsV2_RequestSyntax) can be utilized to retrieve the next 1000 elements .
...
Lets create a function with permissions to both the S3 buckets. 
...
exports.handler = async (event) => {
    ...
    var copyParams = { Bucket: destinationBucket, CopySource: copySource, Key: objectKey[number][i] };
    var data;
    try{
    ...
    const res1 = await copytoS3bucket('<Amplify_Created_Bucket>', '<AWS_Console_created_bucket>', 1);
    const res2 = await copytoS3bucket('<AWS_Console_created_bucket>', '<Amplify_Created_Bucket>', 0);
    return [res1, res2];
};
```
...
### Adding CDK StepFunction using Amplify custom resource
...
export class cdkStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps, amplifyResourceProps?: AmplifyHelpers.AmplifyResourceProps) {
    ...
    const readAmplifyS3 = lambda.Function.fromFunctionArn(this, "readAmplifyS3", readAmplifyS3Arn);

    const readAwsS3Arn = cdk.Fn.ref(retVal.function.readAwsS3.Arn);
    const readAwsS3 = lambda.Function.fromFunctionArn(this, "readAwsS3", readAwsS3Arn);

    const filterS3keysArn = cdk.Fn.ref(retVal.function.filterS3keys.Arn);
    const filterS3keys = lambda.Function.fromFunctionArn(this, "filterS3keys", filterS3keysArn);
    
    const syncS3BucketsArn = cdk.Fn.ref(retVal.function.syncS3Buckets.Arn);
    const syncS3Buckets = lambda.Function.fromFunctionArn(this, "syncS3Buckets", syncS3BucketsArn);
  ...
  const getAmplifykeysTask = new tasks.LambdaInvoke(this, "getAmplifykeystask", {
...
// create a StepFunction flow
  const definition = new sfn.Parallel(this, "ParallelTask",{}).branch(getAWSkeysTask).branch(getAmplifykeysTask).next(compareKeysTask).next(s3syncTask);
...
Lets first upload some objects to S3 buckets.
